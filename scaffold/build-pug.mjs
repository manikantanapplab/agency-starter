...
// ── Step 2: Detect which components a page uses ───────────────
// Scans the page pug for:
//   include ../components/NAME/  → direct include
//   +NAME(  or  +NAME           → mixin call
//   .NAME  or  class="NAME"      → CSS class usage
// Cross-checks against actual component folders with a scss file
...
for (const file of pageFiles) {
  ...
  // Strip all existing block pagecss / componentcss (clean slate)
  content = content
    .replace(/\nblock pagecss\n(?:[ \t]+[^\n]*\n)*/g, '\n')
    .replace(/\nblock componentcss\n(?:[ \t]+[^\n]*\n)*/g, '\n')
  ...
  if (MODE === 'single') {
    ...
    // Auto-detect which components this page uses
    const usedComps = detectPageComponents(content, availableComponents);
